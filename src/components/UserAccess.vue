  ...
  <div class="form-container">
    ...
    <v-card-actions class="mx-5 pt-0">
      <v-row justify="start" class="mb-4">
        <v-btn color="primary" class="mr-3" @click="constructState(formData)">
          Save
        </v-btn>
        <v-btn @click="cancelFunction()"> Cancel </v-btn>
      </v-row>
    </v-card-actions>
  </div>
</template>
...
export default {
  ...
  methods: {
      constructState(state){
          ...
          this.userAccess(sender)
      }
  },
  computed: {
      ...mapState({
          form: (state) => state.users.forms.signUpForm
      })
  }
};
</script>